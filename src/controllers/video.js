...
const videoQueue = require("../queue.js");
...
const extractAudio = async (req, res, handleErr) => {
  ...
  try {
    ...
    if (e.message && e.message.includes('Output file #0 does not contain any stream')) {
      return handleErr({ status: 400, message: "This video does not contain an audio track." });
    }
...
const getVideoAsset = async (req, res, handleErr) => {
  ...
  try {
    ...
    if (e.code === 'ENOENT') {
      return handleErr({ status: 404, message: "Asset not found on disk" });
    }
    console.log(e);
    if (!res.headersSent) {
      return handleErr(e);
